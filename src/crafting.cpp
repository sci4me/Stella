        ...
        bool calculate(Recipe *recipe, u32 count, Item_Container *player_inventory) {
            ...
            for(u32 i = 0; i < recipe->n_inputs; i++) {
                ...
                if(n < needed.count) {
                    ...
                    s32 idx = g_inst->recipes->output_type_to_recipe.index_of(needed.type);
                    if(idx == -1) {
                        missing_something = true;
                    } else {
                        missing_something = missing_something || calculate(g_inst->recipes->output_type_to_recipe.slots[idx].value, r, player_inventory);
                    }
                }
            }
